    ...
    <>
      ...
      <div className="w-full h-screen flex justify-center items-center dark:bg-white/40 bg-[#000]/50">
        ...
        <AnimatePage initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} >
          ...
          <form onSubmit={handleSubmit} action="" className='p-10  max-h-[320px] sm:max-h-[405px] rounded-[5px] dark:bg-white dark:text-gray-500 bg-[#000000BF]  flex flex-col ' >
            ...
            <span onClick={()=>navigate('/forgotpassword')} className='cursor-pointer text-right text-[12px] mt-1 text-[#B3B3B3]'>Forgot Password?</span>
            <div className="flex justify-center w-full h-full items-end mt-[30px]">
              <h4 className='text-[#737373] font-roboto font-semibold text-[15px] mr-1' > New to MovieFlair?</h4> <span className='text-[#fff] dark:text-gray-500 font-roboto font-medium text-[15px] cursor-pointer'onClick={()=> navigate('/signup')} >Sign up now.</span>
            </div>
          </form>
        </AnimatePage>
      </div>
